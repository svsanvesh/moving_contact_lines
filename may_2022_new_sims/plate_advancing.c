...
//refer: http://basilisk.fr/src/two-phase.h
        ...
        fraction (f,  y + lc/(tan(theta0)*exp((x)/lc)));
	boundary ({f});
...
event dumpfile (t = 0.;t += 0.001;t <= T_end)
{
    char name[80];
    sprintf(name,"dump-%g",t*1000);
    scalar pid[];
    foreach()
    {
...
int main()
{
        ...
        origin (0.0, -L0/2.);  // Origin is at the left side centre of the box.
        N = 512;
//        stokes = true;
        f.sigma = surf;
        f.height = h;
