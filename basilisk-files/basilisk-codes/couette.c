 ...
 * The bottom plate is moving and the top plate is stationary */
/* Date: 16-July-2021 */
// Author:  Anvesh  
// Code comments : faulty [But editable with the help of right bpoundary conditions] 
...
int main() {                // Main program begins here
	...
	U0 =2.5;            // Velocity of the bottom plate
...
p[left]    = neumann(0.);  // We give no pressure gradient to check for linear profile 
pf[left]   = neumann(0.0);
...
p[right]   = neumann(0.);  //we give no pressure gradient - couette flow 
pf[right]  = neumann(0. );
...
//	fractions (phi, cs, fs);
	...
	boundary(all);
     ...
     foreach()
	  ...
	  u.x[] = cs[] ? H0  : 0.;	// 	THIS LINE IS COMMENTED TO CHECK IF THE CODE STILL RUNS 
...
event movies (i += 10  ; t <=2)
{
	...
	foreach()
		m[] = cs[]  ;
	boundary ({m});
	output_ppm (omega, file = "vort.mp4", box = {{-L0/2., 0.0},{L0/2., H0}}, 
